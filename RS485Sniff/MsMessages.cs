...
namespace X13.Periphery {
  ...
  public class MsMessage {
    ...
    public static MsMessage Parse(byte[] buf, int start, int end) {
      ...
      case MsMessageType.CONNACK:
      case MsMessageType.SUBACK:
      case MsMessageType.PINGRESP:
        return new MsPingResp(buf, start, end);
  ...
  internal class MsPingResp : MsMessage {
    public MsPingResp(byte[] buf, int start, int end)
      : base(buf, start, end) {
    }

    public override byte[] GetBytes() {
      throw new NotSupportedException("MsPingResp.GetBytes() not supported");
    }
  }
